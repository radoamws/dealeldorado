...
jQuery(document).ready(function ($) {
  ...
  jQuery("#start_prefill_begin").on("click", function () {
    ...
    getPostIds();
    if (cegg_post_ids.length == 0) return false;
    ...
    if (jQuery("#logs").is(":hidden")) jQuery("#logs").slideDown("slow");
...
function getPostIds() {
  ...
  for (var i = 0; i < content_egg_prefill.posts.length; i++) {
    if (jQuery.inArray(content_egg_prefill.posts[i].post_type, post_type) == -1)
      continue;
    if (
      jQuery.inArray(content_egg_prefill.posts[i].post_status, post_status) ==
      -1
    )
      continue;
...
function prefill() {
  ...
  if (cegg_post_ids.length == 0) cegg_p_stop = 1;
  ...
  var progress =
    ((cegg_post_ids_total - cegg_post_ids.length) * 100) / cegg_post_ids_total;
  ...
  cegg_xxx.done(function (data, textStatus) {
    ...
    if (cegg_p_stop == 1 || cmd == "stop") {
      if (cegg_p_stop) jQuery("#start_prefill").prop("disabled", false);
      else jQuery("#start_prefill").prop("disabled", true);
